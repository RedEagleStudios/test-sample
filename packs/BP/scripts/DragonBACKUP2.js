 ...
 */
function checkSolidBlockBelow(player) {
    try {
        const playerLocation = player.location;
        const dimension = player.dimension;
        
        // Check blocks directly below the player (check current Y and up to 4 blocks down)
        // Start from Y=0 (player's feet level) down to Y=-5
        for (let y = 0; y >= -5; y--) {
            const checkLocation = {
        ...
        console.log(`[${player.name}] ✗ No solid block found below (checked Y=${Math.floor(playerLocation.y)} to Y=${Math.floor(playerLocation.y) - 5})`);
        return {
...
export function detectSolidBlockTransition(player) {
    try {
        ...
        // Detailed logging: Check current block data
        const currentBlockData = checkSolidBlockBelow(player);
