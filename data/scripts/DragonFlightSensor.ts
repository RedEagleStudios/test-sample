import { world, system, Entity, Block, Dimension } from "@minecraft/server";
 ...
 */
export function detectDragons(): Entity[] {
    try {
        const dragons: Entity[] = [];
 ...
 */
export function isSolidBlock(block: Block | undefined): boolean {
    if (!block) return false;
    
    // List of non-solid/passable blocks that players can pass through
    const nonSolidBlocks: string[] = [
        "minecraft:air",
        "minecraft:water",
...
interface SolidBlockResult {
    hasSolidBlockBelow: boolean;
    block: Block | null;
 ...
 */
export function checkSolidBlockBelow(entity: Entity, maxDistance: number = 4): SolidBlockResult {
    try {
...
interface SolidBlockState {
    hadSolidBlockBefore: boolean;
}

interface RotationState {
    previousViewX: number;
    previousViewZ: number;
}
 ...
 */
const solidBlockStates = new Map<string, SolidBlockState>();
 ...
 */
function calculateYawFromDirection(x: number, z: number): number {
    // Calculate yaw from x and z components
    let yaw = Math.atan2(x, z) * (180 / Math.PI);
    return yaw;
}

interface YawRotationResult {
    entity: Entity;
    currentYaw: number;
    previousYaw: number;
 ...
 */
export function detectYawRotation(dragon: Entity): YawRotationResult {
    try {
...
interface SolidBlockTransitionResult {
    entity: Entity;
    transitionType: string;
    flightState: string;
    hasSolidBlockNow: boolean;
    hadSolidBlockBefore: boolean;
    blockData: SolidBlockResult | null;
    detectionRange: number;
    isBeforeCondition: boolean;
    isAfterCondition: boolean;
    isStable: boolean;
    isLanding: boolean;
    isAirborne: boolean;
    isGrounded: boolean;
 ...
 * - BEFORE: No solid block below → Solid block appears below (LANDING)
 * - AFTER: Solid block below → No solid block below (TAKEOFF)
 * 
 * @param {Entity} dragon - The dragon entity to check
 * @returns {Object} Object with transition state and block info
 */
export function detectSolidBlockTransition(dragon: Entity): SolidBlockTransitionResult {
    try {
        const dragonId = dragon.id;
        ...
        let currentBlockData: SolidBlockResult;
        ...
        let flightState = "flying"; // Default state
        
        if (!hadSolidBlockBefore && hasSolidBlockNow) {
            // BEFORE condition: No solid block → Solid block appeared (LANDING)
            transitionType = "before";
            flightState = "landing"; // Dragon is landing
            //console.log(`[Dragon ${dragonId}] 🔵 BEFORE TRANSITION: Air → Ground (hadBefore: ${hadSolidBlockBefore}, hasNow: ${hasSolidBlockNow})`);
            
            // Set re_dra:flight_state to "landing"
            try {
                dragon.setProperty("re_dra:flight_state", "landing");
            } catch (propertyError) {
                //console.warn("Error setting re_dra:flight_state to landing:", propertyError);
            }
            
        } else if (hadSolidBlockBefore && !hasSolidBlockNow) {
            // AFTER condition: Solid block → No solid block (TAKEOFF)
            transitionType = "after";
            flightState = "airborne"; // Dragon just left ground
            //console.log(`[Dragon ${dragonId}] 🟢 AFTER TRANSITION: Ground → Air (hadBefore: ${hadSolidBlockBefore}, hasNow: ${hasSolidBlockNow})`);
            
            // Set re_dra:flight_state to "airborne"
            try {
                dragon.setProperty("re_dra:flight_state", "airborne");
            } catch (propertyError) {
                //console.warn("Error setting re_dra:flight_state to airborne:", propertyError);
            }
            
        } else if (hasSolidBlockNow) {
            // Dragon is on ground (stable)
            flightState = "grounded";
            
            // Set re_dra:flight_state to "grounded"
            try {
                dragon.setProperty("re_dra:flight_state", "grounded");
            } catch (propertyError) {
                //console.warn("Error setting re_dra:flight_state to grounded:", propertyError);
            }
            
        } else {
            // Dragon is in air (stable flying)
            flightState = "flying";
            
            // Set re_dra:flight_state to "flying"
            try {
                dragon.setProperty("re_dra:flight_state", "flying");
            } catch (propertyError) {
                //console.warn("Error setting re_dra:flight_state to flying:", propertyError);
            }
        }
        ...
        return {
            ...
            isAirborne: flightState === "airborne",
            isGrounded: flightState === "grounded",
            isFlying: flightState === "flying"
        };
    } catch (error) {
 ...
 */
function dragonFlightTick(): void {
    try {
        // Get all dragons
        ...
        for (const dragon of dragons) {
            ...
            if (transitionData.isBeforeCondition) {
                ...
                //console.log(`🛬 Dragon ${dragon.id} landed on solid ground (LANDING)`);
                //console.log(`   Block: ${transitionData.blockData.block?.typeId}`);
                //console.log(`   Distance: ${transitionData.blockData.distance} blocks`);
                //console.log(`   Flight state: ${transitionData.flightState} (landing)`);
            }
            ...
            if (transitionData.isAfterCondition) {
                ...
                //console.log(`🛫 Dragon ${dragon.id} left solid ground (TAKEOFF)`);
                //console.log(`   Was on block, now airborne`);
                //console.log(`   Flight state: ${transitionData.flightState} (airborne)`);
            }
            
            // Handle stable states
            if (transitionData.isStable) {
                if (transitionData.isGrounded) {
                    // Dragon is stable on ground
                    //console.log(`   Dragon stable on ground - Flight state: grounded`);
                } else if (transitionData.isFlying) {
                    // Dragon is stable in air (flying)
                    //console.log(`   Dragon stable in air - Flight state: flying`);
                }
            }
 ...
 */
function initializeDragonFlightSystem(): void {
