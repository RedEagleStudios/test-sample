 ...
 */
export function detectDragonRiders(): Player[] {
    try {
        const dragonRiders: Player[] = [];
 ...
 */
export function checkSolidBlockBelow(player: Player, maxDistance: number = 4): SolidBlockResult {
    try {
 ...
 * Key: player.name, Value: { previousViewX: number, previousViewZ: number }
 */
...
/**
 * Modular function to detect player's yaw rotation direction using view direction
 * Can be reused in other scripts by importing this function
 * @param {Player} player - The player to check
 * @returns {Object} Object with rotation direction and view data
 */
export function detectYawRotation(player: Player): YawRotationResult {
    try {
        ...
        // Get or create player rotation state
        let rotationState = rotationStates.get(playerName);
        ...
        if (!rotationState) {
            ...
            rotationStates.set(playerName, rotationState);
        }
        ...
        if (rotationDirection !== "none") {
            //console.log(`[ROTATION ${rotationDirection.toUpperCase()}] Player: ${playerName}`);
        ...
        // Update player's view direction in rotation state
        ...
        console.warn(`Error detecting yaw rotation for player ${player.name}:`, error);
        return {
            player: player,
            currentYaw: 0,
            previousYaw: 0,
 ...
 */
export function detectSolidBlockTransition(player: Player): SolidBlockTransitionResult {
    try {
        const playerName = player.name;
        const playerLoc = player.location;
        
        // Get previous solid block state (default to false if new player)
        const previousState = solidBlockStates.get(playerName) || { hadSolidBlockBefore: false };
        ...
        solidBlockStates.set(playerName, { hadSolidBlockBefore: hasSolidBlockNow });
 ...
 * Detects dragon riders and checks for solid block transitions and yaw rotation
 */
function dragonRiderTick(): void {
    try {
        ...
        for (const rider of dragonRiders) {
            ...
            // Detect yaw rotation for this rider
            const yawData = detectYawRotation(rider);
            
            // Handle looking left
            if (yawData.isLookingLeft) {
                rider.onScreenDisplay.setActionBar(`§dLooking Left`);
                //console.log(`[ROTATION LEFT] Player: ${rider.name}`);
            ...
            if (yawData.isLookingRight) {
                rider.onScreenDisplay.setActionBar(`§bLooking Right`);
                //console.log(`[ROTATION RIGHT] Player: ${rider.name}`);
                //console.log(`  - Current Yaw: ${yawData.currentYaw.toFixed(2)}°`);
                //console.log(`  - Previous Yaw: ${yawData.previousYaw.toFixed(2)}°`);
                //console.log(`  - Yaw Difference: ${yawData.yawDifference.toFixed(2)}°`);
                //console.log(`  - View Direction: x=${yawData.viewDirection.x.toFixed(3)}, z=${yawData.viewDirection.z.toFixed(3)}`);
            ...
            if (yawData.isStable) {
                // Player is not rotating or rotation is below threshold
                // This is when the player stops turning their head (neutral/stable state)
                // Add your reset/stable logic here
                // Example: Reset dragon steering to neutral, clear previous steering commands, etc.
                //rider.onScreenDisplay.setActionBar("§7Stable");
            }
 ...
 * Starts the 1-tick interval for detecting riders, solid blocks, and yaw rotation
 */
function initializeDragonSystem(): void {
    //console.log("Dragon rider system initialized - Running every tick");
